...
public class A3mCa08 extends PApplet {
...
public void setup() {
    ...
    for (int u = 0; u < 19; ++u) { 
    	...
    	for (int i = 0; i < 17; ++i) {
			...
			.noStroke()
			.fill(colors[1])
			.size( map(PApplet.parseFloat(myData[u][i]), 0, 4000, 15, 40) )
			...
			.noStroke()
			.fill(colors[2])
			.size(map(PApplet.parseFloat(myData1[u][i]), 0, 4000, 10, 30))
			...
			.noStroke()
			.fill(colors[3])
			.size(map(PApplet.parseFloat(myData2[u][i]), 0, 4000, 10, 30))
			.loc( anclaX+(60*(i)-10), anclaY+(60*(u)) )
			.anchorAt(H.CENTER)
    		;
...
public void mousePressed() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "A3mCa08" };
    if (passedArgs != null) {
